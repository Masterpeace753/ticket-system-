...
Only use when `mcp_awesome-copil_*` tools are confirmed available. If not present, skip this functionality entirely.
...
**If tools are NOT available:** Skip all `/suggest` functionality, focus only on local scaffolding.

**If tools ARE available:** Proactively suggest relevant resources after `/bootstrap`, include collection recommendations in validation reports.
